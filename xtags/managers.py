...
from xtags.models import TaggedItem, GenericTaggedItemBase
from xtags.utils import require_instance_manager
from xtags.widgets import TagAutoCompleteInput
...
class _TaggableManager(models.Manager):
    ...
    @require_instance_manager
    def similar_dict(self, strict=False):
        ...
        if len(lookup_keys) == 1:
            ...
            for result in qs[:100]:
                ct = ContentType.objects.get_for_id(result[lookup_keys[0]])
                items.setdefault(result['content_type'], [])
                if strict:
                    if ct.model_class() == self.model:
                        items[result['content_type']].append(result)
                else:
                    items[result['content_type']].append(result)
